...
export default class SignUp extends Component {
    ...
    constructor() {
        super()
        this.state = { email: 'aman@yopmail.com', password: '11111111', confirmPassword: '11111111', modalVisible: false, focusInput: false, loader: false };
    }
    ...
    validate() {
        ...
        if (email.length === 0) {
            ...
            Toasty.show('Password should be atleast 6 characters long');
        }
        else if (confirmPassword.length === 0) {
            Toasty.show('Confirm password field can\'t be empty');
        }
        else if (password != confirmPassword) {
            Toasty.show('Password and confirm password doesn\'t match');
        }
        else {
    ...
    signUpApiCall() {
        ...
        ApiCaller.call('users/', "POST", data, false)
            .then((response) => {
                EventRegister.emit('loader', false)

                if (response) {
                    console.log(response)
                    Toasty.show(response.message, 1500)
                            ...
                            <ScrollView style={{ width: '100%', height: '100%' }} showsVerticalScrollIndicator={false}>
                                <View style={[{ width: '100%', marginBottom: 50 }]}>
                                    ...
                                    <View style={[styles.viewStyle]}>
                                        ...
                                        <View style={[styles.editView, { marginTop: 10 }]}>
                                            <Text style={styles.editText}>Confirm Password</Text>
                                            <TextInput
                                                style={styles.inputStyle}
                                                placeholder="Enter Password"
                                                secureTextEntry={true}
                                                ...
                                                placeholderTextColor='black'
                                                blurOnSubmit={true}
                                                onChangeText={value => this.setState({ confirmPassword: this.removeEmojis(value.replace(/\s/g, '')) })}
                                                returnKeyType='done'
                                        ...
                                        <View style={{ alignSelf: 'center', marginTop: 30 }}>
                                            <Text style={styles.signUpTermPolicy}>By clicking on sign up button, you agree with our <Text style={[styles.signUpTermPolicy, { fontFamily: 'HelveticaNeueLTStd-Md' }]} onPress={() => this.onPressCall('TermCondition')}>terms & use</Text> and <Text style={[styles.signUpTermPolicy, { fontFamily: 'HelveticaNeueLTStd-Md' }]} onPress={() => this.onPressCall('PrivacyPolicy')}>privacy policy.</Text></Text>
                                        </View>
                                ...
                                <Text style={[styles.signUp, { textAlign: 'center' }]}>Already have an account? <Text onPress={() => this.onPressCall('Login')} style={[styles.signUp, { fontFamily: 'HelveticaNeueLTStd-Md' }]}>Sign In</Text></Text>
