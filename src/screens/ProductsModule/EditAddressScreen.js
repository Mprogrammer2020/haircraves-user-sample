...
export default class EditAddressScreen extends Component {
    ...
    componentDidMount() {
        ...
        this.props.navigation.addListener(
            ...
            () => {
                ...
                var addressID = navigation.getParam('addressID');
    ...
    getAddress(addressID) {
        ...
        ApiCaller.call('users/address/'+addressID, "GET", null, true)
            .then((response) => {
                EventRegister.emit('loader', false)
                if (response) {
                    console.log("address ===>>>>", response.address[0].address)

                    this.setState({addressName : response.address[0].address})
    ...
    addCardApiCall() {
        ...
        EventRegister.emit('loader', true)
        ApiCaller.call('users/address/'+this.state.address_id, "PUT", data, true)
            .then((response) => {
                EventRegister.emit('loader', false)
                                ...
                                <View style={[styles.viewStyle, { marginTop: 20 }]}>
                                    ...
                                    <View style={{ width: '100%', height: '100%' }}>
                                        <View style={{ flex: 1 }}>
                                            ...
                                            <View style={[styles.editView, { marginTop: 50 }]}>
                                                ...
                                                <TextInput
                                                    ...
                                                    blurOnSubmit={true}
                                                    onChangeText={(addressName) => this.setState({ addressName: !this.state.addressName ? addressName.replace(/\s/g, '') : this.removeEmojis(addressName) })}
                                                    returnKeyType='done'
                                            ...
                                            <TouchableOpacity activeOpacity={1} style={[styles.buttonStyle, { width: '90%', alignSelf: 'center', alignItems: 'center', marginTop: 50 }]}
